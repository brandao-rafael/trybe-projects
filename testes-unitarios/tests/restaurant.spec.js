...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    expect(typeof objmenu.fetchMenu).toBe('function')
    ...
    // ```
    expect(createMenu({ food: {}, drink: {},}).fetchMenu()).toEqual({food: expect.any(Object), drink: expect.any(Object),});
    ...
    // ```
    expect(createMenu({ food: { coxinha: 3.9 }, drink: { cerveja: 6.9 } }).fetchMenu()).toEqual({ food: { coxinha: 3.9 }, drink: { cerveja: 6.9 } })
  ...
  // ```
    expect(createMenu().consumiption).toStrictEqual([])
  ...
  // ```
  const objetoRetornado = createMenu({ food: { coxinha: 3.9 }, drink: { cerveja: 6.9 } });
  objetoRetornado.order("coxinha");
  // objetoRetornado.consumption // Retorno: ["coxinha"]
  // ```
    expect(objetoRetornado.consumiption).toStrictEqual(['coxinha'])
  ...
  // ```
  const order1 = createMenu({ food: { coxinha: 3.9 }, drink: { cerveja: 6.9 } });
  order1.order('coxinha');
  order1.order('agua');
  order1.order('sopa');
  order1.order('sashimi');
  expect(order1.consumiption).toEqual(['coxinha', 'agua', 'sopa', 'sashimi']);
  ...
  // ```
  const order2 = createMenu({ food: { coxinha: 3.9 }, drink: { cerveja: 6.9 } });
  order2.order('coxinha');
  order2.order('agua');
  order2.order('coxinha');
  expect(order2.consumiption).toEqual(['coxinha', 'agua', 'coxinha']);
  ...
  // ```
  const order3 = createMenu({ food: { coxinha: 3.9 }, drink: { cerveja: 6.9 , agua: 2.5 } });
  order3.order('coxinha');
  order3.order('agua');
  order3.order('coxinha');;
  expect(order3.pay()).toBe(11.33);
